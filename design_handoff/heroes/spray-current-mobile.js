...
(() => {
  ...
  /* ============================== CONFIG ================================ */
  const CONFIG = {
    PARTICLE_TEX: 320,         // particle count = TEX^2 (320 -> 102,400) — mobile budget
    SIM_RESOLUTION: 96,        // mobile budget
    ...
    PRESSURE: 0.8,
    PRESSURE_ITERATIONS: 14,   // mobile budget
    CURL: 11,
    SPLAT_RADIUS: 0.005,
    SPLAT_FORCE: 2600,
    ADVECT_SCALE: 0.7,
    ...
    EMIT_MAX: 0.0045,
    EMIT_BUDGET: 0.20,
    HOLD_MULT: 5.0,            // touch boost: hold-pour streams much heavier (stir is scroll on touch)
    POUR_MAX: 0.014,           // per-frame cap for the held pour (above EMIT_MAX drag trickle)
    POUR_SPREAD: 0.10,         // held pour lands wider than the drag trickle
    BUDGET_TAPER: 0.3,
    EMIT_SPREAD: 0.048,
    BURST_FRACTION: 0.12,      // touch boost: tap announces with a much bigger pour
    BURST_SPREAD: 0.085,
    PULSE_SPLATS: 16,
    PULSE_FORCE: 800,
  ...
  class SprayCurrent extends HTMLElement {
    ...
    _run(canvas, gl) {
      ...
      function resizeCanvas() {
        const dpr = Math.min(window.devicePixelRatio || 1, 1.5); // mobile budget
        ...
        if ((canvas.width !== w || canvas.height !== h) && w > 0 && h > 0) {
          ...
          const jitter = canvas.width === w && canvas.height > 0 && Math.abs(h - canvas.height) / canvas.height < 0.25;
        ...
        if (!self._rm) {
          for (let i = 0; i < CONFIG.PULSE_SPLATS; i++) {
            ...
            fluidSplat(cx + Math.cos(ang) * 0.015, cy + Math.sin(ang) * 0.015,
                       Math.cos(ang) * CONFIG.PULSE_FORCE, Math.sin(ang) * CONFIG.PULSE_FORCE, 2.2);
          }
        } else {

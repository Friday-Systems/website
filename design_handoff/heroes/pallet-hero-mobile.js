...
(function () {
  ...
  class PalletHero extends HTMLElement {
    ...
    _start() {
      ...
      renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 1.5)); // mobile perf budget
      ...
      var camScale = 1; // portrait dolly-out factor, set in resize()
      ...
      var PUSH_GAIN = 26;
      var HOLD_MS = 280; // mobile: scroll-flick touchdowns must never read as holds
      ...
      function resize() {
        ...
        camera.aspect = w / h;
        // portrait framing: dolly out so the full stack + scatter radius fits the tall
        // frame — camera moves, physics and box scale untouched (boxes stay readable)
        camScale = camera.aspect < 1 ? Math.min(1.8, 1 + (1 - camera.aspect) * 0.85) : 1;

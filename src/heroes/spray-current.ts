/* <spray-current> — grain-fluid memory background.
   ...
   on re-entry, and the `paused` property to gate the sim while occluded.
   ...
   Keeps: prefers-reduced-motion static field, hidden-tab pause, WebGL2 CSS
   fallback (host's own navy gradient).

   Build-phase change vs the design prototype: the desktop and mobile forks are
   merged — the touch tier (IS_TOUCH, coarse pointer) selects the mobile
   budgets (particles 320², sim 96, pressure iters 14, DPR 1.5) and the touch
   interaction boosts (heavier hold-pour + bigger tap burst, since stir is
   scroll on touch), plus the browser-chrome resize-jitter guard. All values
   are the fork's, verbatim. */
...
/* ============================== CONFIG ================================ */
const CONFIG = {
  PARTICLE_TEX: COARSE ? 320 : 448, // particle count = TEX^2 (448 -> 200,704; mobile 320 -> 102,400)
  ...
  EMIT_MAX: 0.0045,
  EMIT_BUDGET: 0.2,
  HOLD_MULT: COARSE ? 5.0 : 1.6, // touch boost: hold-pour streams much heavier (stir is scroll on touch)
  POUR_MAX: COARSE ? 0.014 : 0.0045, // per-frame cap for the held pour (desktop = EMIT_MAX)
  POUR_SPREAD: COARSE ? 0.1 : 0.048, // held pour lands wider on touch (desktop = EMIT_SPREAD)
  BUDGET_TAPER: 0.3,
  EMIT_SPREAD: 0.048,
  BURST_FRACTION: COARSE ? 0.12 : 0.05, // touch boost: tap announces with a much bigger pour
  BURST_SPREAD: COARSE ? 0.085 : 0.045,
  PULSE_SPLATS: COARSE ? 16 : 14,
  PULSE_FORCE: COARSE ? 800 : 550,
  PULSE_RADIUS: COARSE ? 2.2 : 1.5,
  DPR_CAP: COARSE ? 1.5 : 2,
...
const STYLES = [
  ...
  { v1: [1.0, 0.404, 0.385], v2: [0.845, 0.183, 0.574] },
  { v1: [0.574, 0.827, 0.425], v2: [0.0, 0.729, 0.634] },
  { v1: [0.909, 0.323, 0.878], v2: [0.463, 0.295, 0.9] },
];
/* ====================================================================== */
...
class SprayCurrent extends HTMLElement {
  ...
  _rm = false;
  _seeded = false;
  _styleIndex: number | undefined = undefined;
  _reseed: (() => void) | null = null;
  ...
  press(_clientX?: number, _clientY?: number) {} /* replaced after GL init */
  release() {}
  emit(_x?: number, _y?: number, _fraction?: number, _spread?: number, _x2?: number, _y2?: number) {}
  nudge(_x?: number, _y?: number, _fx?: number, _fy?: number, _r?: number) {}
  reset() {}
  setObstacles(_rects?: ObstacleRect[]) {}
  ...
  _run(canvas: HTMLCanvasElement, gl: WebGL2RenderingContext) {
    const self = this;
    ...
    function program(vsSrc: string, fsSrc: string, attrName: string) {
    ...
    function initialColorData() {
      ...
      for (let i = 0; i < COUNT; i++) {
        if (Math.random() < CONFIG.SPARKLE_SHARE) {
          ...
          d[i * 4 + 2] = 0.9 + 0.35 * (s.v1[2] + (s.v2[2] - s.v1[2]) * t);
        } else {
          const t = Math.random(),
            lum = rand(0.7, 1.05);
    ...
    let velocity: DoubleFBO, divergenceFBO: FBO, curlFBO: FBO, pressureFBO: DoubleFBO, posFBO: DoubleFBO, colorFBO: DoubleFBO;
    ...
    function resizeCanvas() {
      ...
      if ((canvas.width !== w || canvas.height !== h) && w > 0 && h > 0) {
        // mobile browser-chrome collapse jitters the height on every scroll
        // reversal; a full field re-init there wipes all motion mid-scroll.
        // Only rebuild the sim on real size changes (width, or height > 25%).
        const jitter =
          COARSE && canvas.width === w && canvas.height > 0 && Math.abs(h - canvas.height) / canvas.height < 0.25;
        canvas.width = w;
        canvas.height = h;
        return !jitter;
      }
    ...
    /* ---------------- fluid step ---------------- */
    function fluidSplat(x: number, y: number, dx: number, dy: number, radiusScale?: number) {
      P.splat.bind();
    ...
    function setObsUniforms(prog: { u: Record<string, WebGLUniformLocation | null> }) {
    ...
    self.setObstacles = (list?: ObstacleRect[]) => {
      if (self._rm || !list || !list.length) {
        obsState.n = 0;
        return;
      }
      ...
      const aspect = canvas.width / canvas.height;
      const simW = 1 / velocity.texelSizeX,
        simH = 1 / velocity.texelSizeY;
      const cl = (v: number) => Math.max(-CONFIG.OBS_VEL_MAX, Math.min(CONFIG.OBS_VEL_MAX, v));
    ...
    const burstQueue: Burst[] = [];
    let burstCursor = Math.random();
    function enqueueBurst(fraction: number, x: number, y: number, spread: number, x2?: number, y2?: number) {
      const s = STYLES[self._styleIndex!];
      const a = burstCursor;
    ...
    function particleStep(dt: number, now: number) {
    ...
    self.press = (clientX: number, clientY: number) => {
      self._seeded = true;
      pointer.down = true;
      ...
      if (!self._rm) {
        for (let i = 0; i < CONFIG.PULSE_SPLATS; i++) {
          ...
          fluidSplat(
            cx + Math.cos(ang) * 0.015,
            cy + Math.sin(ang) * 0.015,
            Math.cos(ang) * CONFIG.PULSE_FORCE,
            Math.sin(ang) * CONFIG.PULSE_FORCE,
            CONFIG.PULSE_RADIUS
          );
        }
      } else {
    ...
    self.emit = (clientX: number, clientY: number, fraction: number, spread?: number, clientX2?: number, clientY2?: number) => {
      if (self._rm) return;
      const rect = canvas.getBoundingClientRect();
      if (!rect.width || !rect.height) return;
      const cx = (clientX - rect.left) / rect.width;
    ...
    self.nudge = (clientX: number, clientY: number, fx: number, fy: number, radiusScale?: number) => {
    ...
    function frame() {
      if (self._dead) {
        getTicker().remove(frame);
        return;
      }
      if (self._paused || document.hidden) {
      ...
      if (pointer.down && pointer.seeded) {
        const travel = Math.hypot(pointer.x - pointer.px, pointer.y - pointer.py);
        const fraction = Math.min(CONFIG.POUR_MAX, CONFIG.EMIT_BASE * CONFIG.HOLD_MULT + travel * CONFIG.EMIT_PER_UV);
        enqueueBurst(fraction, pointer.x, pointer.y, CONFIG.POUR_SPREAD, pointer.px, pointer.py);
        pointer.px = pointer.x;
        pointer.py = pointer.y;
      }
      fluidStep(dt);
...
if (!customElements.get('spray-current')) customElements.define('spray-current', SprayCurrent);

export type { SprayCurrent };
